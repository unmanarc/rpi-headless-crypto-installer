...
echo "${1}" >> /tmp/rpisystem/update.sh
...
dd if=/dev/urandom of="${TARGET_PART2}" bs=1M count=8 &> /dev/null
if [ "$?" != "0" ]; then
        echo -e "${BIWhite}[+] Error: aborting.${Color_Off}"
        cleanup "${TARGET_IMG}"
        exit
...
cryptsetup --force-password -q -v ${LUKS_FORMAT_OPTIONS} luksFormat "${TARGET_PART2}"
if [ "$?" != "0" ]; then
        echo -e "${BIWhite}[+] Error: aborting, Can't create the encrypted container.${Color_Off}"
        cleanup "${TARGET_IMG}"
        exit
fi
...
if [ "$?" != "0" ]; then
        echo -e "${BIWhite}[+] Error: aborting, Can't open the encrypted container ${TARGET_PART2} ${Color_Off}"
        cleanup "${TARGET_IMG}"
        exit
fi

echo -e "${BIWhite}[+] Copying the boot partition from ${TARGET_IMG} to ${TARGET_PART1}${Color_Off}"
partclone.fat32 --dev-to-dev --source "${LOOPDEV}p1" -O "${TARGET_PART1}"
